...
export default function SupportPage() {
  ...
  const handleBuyTokens = () => {
    const message = encodeURIComponent(
      `Hello WorkaHive Support,%0A%0AI want to purchase tokens.%0A%0AToken Plan: ${tokens}%0ANote: ${note}`
    );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="bg-white w-full max-w-2xl p-8 rounded-2xl shadow-md text-center space-y-6">
        ...
        <button
          onClick={() =>
            window.open(
              `https://wa.me/${whatsappNumber}?text=Hello WorkaHive Support`,
              "_blank"
            )
          }
        ...
        {isEmployer && (
          <div className="text-left mt-6 border-t pt-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { value: "50", price: "₦5,000" },
                { value: "100", price: "₦9,000" },
                { value: "200", price: "₦17,000" },
              ].map((plan) => (
                <button
                  key={plan.value}
                  onClick={() => setTokens(plan.value)}
